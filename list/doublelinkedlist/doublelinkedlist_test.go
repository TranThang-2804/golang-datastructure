...
func TestListContains(t *testing.T) {
	...
	if actualValue := list.Contains("c"); actualValue != false {
		t.Errorf("Got %v expected %v", actualValue, false)
	}
}
...
func TestListInsert(t *testing.T) {
	...
	if actualValue := list.GetSize(); actualValue != 5 {
		t.Errorf("Got %v expected %v", actualValue, 5)
	}
	if actualValue, expectedValue := strings.Join(list.GetAllNode(), ""), "abcdg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	list.Insert(4, "e", "f") // last to first traversal
	if actualValue := list.GetSize(); actualValue != 7 {
		t.Errorf("Got %v expected %v", actualValue, 7)
	}
	if actualValue, expectedValue := strings.Join(list.GetAllNode(), ""), "abcdefg"; actualValue != expectedValue {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
}
...
func benchmarkGet(b *testing.B, list *DoubleLinkedList[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Get(n)
		}
...
func benchmarkAdd(b *testing.B, list *DoubleLinkedList[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Append(n)
		}
...
func benchmarkRemove(b *testing.B, list *DoubleLinkedList[int], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			list.Remove(n)
		}
